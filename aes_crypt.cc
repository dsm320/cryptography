/**
 * aes_crypt.cc
 * Crypt_aes demonstrates how to use symettric AES encryption/decryption on a
 * chunk of data of arbitrary size.
 * 
 * Note: AES keys are usually ephemeral - they are made, shared, used briefly, and discarded.
 * This demo saves an AES key to a file to show that it works, but generally, saving AES keys
 * is a bad practice
 */
...
void parse_args(int argc, char **argv, arg_t &args) {
    long opt;
    while ((opt = getopt(argc, argv, "k:i:o:deg:h")) != -1) {
        switch (opt) {
        case 'k':
